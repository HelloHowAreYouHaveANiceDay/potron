...
export default class PointCloudSM {
  ...
  setLight(light) {
    ...
    const fov = (180 * light.angle) / Math.PI;
    const aspect = light.shadow.mapSize.width / light.shadow.mapSize.height;
    const near = 0.1;
    const far = light.distance === 0 ? 10000 : light.distance;
    this.camera = new THREE.PerspectiveCamera(fov, aspect, near, far);
    this.camera.up.set(0, 0, 1);
    this.camera.position.copy(light.position);

    const target = new THREE.Vector3().addVectors(light.position, light.getWorldDirection(new THREE.Vector3()));
