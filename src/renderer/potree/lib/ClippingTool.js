...
export default class ClippingTool extends EventDispatcher {
  constructor(viewer) {
    ...
    this.viewer.inputHandler.addEventListener('delete', (e) => {
      const volumes = e.selection.filter(e => (e instanceof ClipVolume));
      volumes.forEach(e => this.viewer.scene.removeClipVolume(e));
      const polyVolumes = e.selection.filter(e => (e instanceof PolygonClipVolume));
  ...
  startInsertion(args = {}) {
    ...
    const canvasSize = this.viewer.renderer.getSize();
