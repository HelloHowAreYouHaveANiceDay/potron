...
// const GeoTIFF = (function (exports) {
...
class Reader {
  ...
  static read(data) {
    // const endiannessTag = String.fromCharCode(...Array.from(data.slice(0, 2)));
    // const endianness = Endianness.fromValue(endiannessTag);
    ...
    while (IFDsRead || i < 100) {
      ...
      for (let i = 0; i < numEntries; i++) {
        const tag = Tag.fromValue(entryBuffer.readUInt16LE(i * 12));
        const type = Type.fromValue(entryBuffer.readUInt16LE(i * 12 + 2));
        const count = entryBuffer.readUInt32LE(i * 12 + 4);
        const offsetOrValue = entryBuffer.readUInt32LE(i * 12 + 8);
        ...
        if (valueBytes <= 4) {
          ...
          const ArrayType = typeMapping.get(type);
        ...
        const ifd = new IFDEntry(tag, type, count, offsetOrValue, value);
    ...
    const width = ifdForTag(Tag.IMAGE_WIDTH, ifds).value;
    const height = ifdForTag(Tag.IMAGE_HEIGHT, ifds).value;
    // const compression = ifdForTag(Tag.COMPRESSION, ifds).value;
    const rowsPerStrip = ifdForTag(Tag.ROWS_PER_STRIP, ifds).value;
    const ifdStripOffsets = ifdForTag(Tag.STRIP_OFFSETS, ifds);
    const ifdStripByteCounts = ifdForTag(Tag.STRIP_BYTE_COUNTS, ifds);

    const numStrips = Math.ceil(height / rowsPerStrip);

    const stripByteCounts = [];
    for (let i = 0; i < ifdStripByteCounts.count; i++) {
      const type = ifdStripByteCounts.type;
      const offset = ifdStripByteCounts.offset + i * type.bytes;
    ...
    for (let i = 0; i < ifdStripOffsets.count; i++) {
      const type = ifdStripOffsets.type;
      const offset = ifdStripOffsets.offset + i * type.bytes;
    ...
    for (let i = 0; i < numStrips; i++) {
      ...
      const stripBytes = stripByteCounts[i];
...
class Exporter {
  ...
  static toTiffBuffer(image, params = {}) {
    ...
    const ifdEntryBuffers = new Map();
    const ifdValueBuffers = new Map();
    for (const ifd of ifds) {
      const entryBuffer = new ArrayBuffer(12);
      const entryView = new DataView(entryBuffer);

      // const valueBytes = ifd.type.bytes * ifd.count;
      ...
      if (ifd.count === 1 && ifd.type.bytes <= 4) {
        ...
        valueOffset += valueBuffer.byteLength;
      }
    ...
    return { width, height, buffer: tiffBuffer };
  }
}
...
const _IFDEntry = IFDEntry;
export { _IFDEntry as IFDEntry };
const _Image = Image;
export { _Image as Image };
const _Reader = Reader;
export { _Reader as Reader };
const _Exporter = Exporter;
export { _Exporter as Exporter };
