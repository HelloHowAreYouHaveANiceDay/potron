...
export default class ScreenBoxSelectTool extends EventDispatcher {
  constructor(viewer) {
    super();
  ...
  startInsertion() {
    ...
    const drag = (e) => {
      ...
      const screenCentroid = new THREE.Vector2().addVectors(e.drag.end, e.drag.start).multiplyScalar(0.5);
      const ray = Utils.mouseToRay(screenCentroid, camera, size.width, size.height);

      const diff = new THREE.Vector2().subVectors(e.drag.end, e.drag.start);
    ...
    const drop = (e) => {
      ...
      // const line = new THREE.Line3(ray.origin, new THREE.Vector3().addVectors(ray.origin, ray.direction));
      ...
      for (const pointcloud of this.viewer.scene.pointclouds) {
        ...
        const ray = new THREE.Ray(volCam.getWorldPosition(new THREE.Vector3()), volCam.getWorldDirection(new THREE.Vector3()));
        const rayInverse = new THREE.Ray(
        ...
        const pointsNear = pointcloud.pick(viewer, volCam, ray, pickerSettings);
      ...
      if (allPointsNear.length > 0 && allPointsFar.length > 0) {
        ...
        const closestOnLine = allPointsNear.map(p => viewLine.closestPointToPoint(p.position, false, new THREE.Vector3()));
        const closest = closestOnLine.sort((a, b) => ray.origin.distanceTo(a) - ray.origin.distanceTo(b))[0];

        const farthestOnLine = allPointsFar.map(p => viewLine.closestPointToPoint(p.position, false, new THREE.Vector3()));
