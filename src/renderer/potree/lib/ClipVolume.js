...
export default class ClipVolume extends THREE.Object3D {
  constructor(args) {
    super();
    ...
    const planeFrameGeometry = new THREE.Geometry();
    {
      // middle line
    ...
    const createArrow = (name, direction, color) => {
      ...
      const shaftGeometry = new THREE.Geometry();
  ...
  offset(args) {
    ...
    this.dispatchEvent({ type: 'clip_volume_changed', viewer, volume: this });
  }
  ...
  rotate(args) {
    ...
    if (cs === 'local') {
      ...
      const invM = new THREE.Matrix4().getInverse(this.matrixWorld);
  ...
  updateLocalSystem() {
    ...
    const rotQuat = this.getWorldQuaternion();
