...
import NormalizationMaterial from './NormalizationMaterial';
import NormalizationEDLMaterial from './NormalizationEDLMaterial';
...
export default class HQSplatRenderer {
  ...
  resize() {
    ...
    const width = viewer.renderer.getSize().width;
    const height = viewer.renderer.getSize().height;
  ...
  render() {
    ...
    for (const pointcloud of visiblePointClouds) {
      ...
      if (!this.attributeMaterials.has(pointcloud)) {
        const attributeMaterial = new PointCloudMaterial();
    ...
    { // DEPTH PASS
      for (const pointcloud of visiblePointClouds) {
        ...
        const material = originalMaterials.get(pointcloud);
    ...
    { // NORMALIZATION PASS
      const normalizationMaterial = this.useEDL ? this.normalizationEDLMaterial : this.normalizationMaterial;
