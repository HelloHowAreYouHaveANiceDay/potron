...
export default class TransformationTool {
  constructor(viewer) {
    this.viewer = viewer;
  ...
  initializeScaleHandles() {
    ...
    for (const handleName of Object.keys(this.scaleHandles)) {
      ...
      pickSphere.addEventListener('mouseover', () => {
        // node.setOpacity(1);
      });
      ...
      pickSphere.addEventListener('mouseleave', () => {
        // node.setOpacity(0.4);
      });
    }
  }
  ...
  initializeFocusHandles() {
    ...
    const texture = new THREE.TextureLoader().load('static/resources/icons/eye_2.png');
    ...
    for (const handleName of Object.keys(this.focusHandles)) {
      ...
      pickSphere.addEventListener('drag', () => { });
