...
class Measure extends THREE.Object3D {
      ...
      const drag = (e) => {
        ...
        if (I) {
          ...
          if (i !== -1) {
            ...
            for (const key of Object.keys(I.point).filter(e => e !== 'position')) {
  ...
  getArea() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      const p2 = this.points[j].position;
  ...
  getTotalDistance() {
    ...
    for (let i = 1; i < this.points.length; i++) {
      ...
      const curr = this.points[i].position;
    ...
    if (this.closed && this.points.length > 1) {
      const first = this.points[0].position;
  ...
  getAngle(index) {
    ...
    const previous = (index === 0) ? this.points[this.points.length - 1] : this.points[index - 1];
    const point = this.points[index];
  ...
  update() {
    ...
    for (let i = 0; i <= lastIndex; i++) {
      ...
      { // edge labels
        ...
        const distance = point.position.distanceTo(nextPoint.position);
    ...
    { // update height stuff
      ...
      if (this.showHeight) {
        ...
        const msg = `${Utils.addCommas(height.toFixed(2))} ${this.lengthUnit.code}`;
    ...
    { // update area label
      ...
      const msg = `${Utils.addCommas(this.getArea().toFixed(1))} ${this.lengthUnit.code}\u00B2`;
