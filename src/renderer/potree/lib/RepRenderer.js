...
export default class RepRenderer {
  constructor(viewer) {
    this.viewer = viewer;
  ...
  resize() {
    ...
    const needsResize = (this.rtColor.width !== width || this.rtColor.height !== height);
  ...
  render() {
    ...
    const query = Potree.startQuery('stuff', viewer.renderer.getContext());
    ...
    if (!this.disableSnapshots) {
      ...
      const query = Potree.startQuery('create snapshot', viewer.renderer.getContext());
      ...
      for (const octree of viewer.scene.pointclouds) {
        ...
        const from = this.history.version * (octree.visibleNodes.length / this.history.maxSnapshots);
        const to = (this.history.version + 1) * (octree.visibleNodes.length / this.history.maxSnapshots);

        // DEBUG!!!
        // let from = 0;
        // let to = 20;
        const nodes = octree.visibleNodes.slice(from, to);
    ...
    {
      const query = Potree.startQuery('render snapshots', viewer.renderer.getContext());
      ...
      for (const octree of viewer.scene.pointclouds) {
        if (!this.disableSnapshots) {
          ...
          const uniforms = octree.material.uniforms;
          if (this.history.snapshots.length === this.history.maxSnapshots) {
        ...
        const nodes = octree.visibleNodes.slice(0, 50);
    ...
    { // EDL OCCLUSION PASS
      const query = Potree.startQuery('EDL', viewer.renderer.getContext());
