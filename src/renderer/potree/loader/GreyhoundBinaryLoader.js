...
export class GreyhoundBinaryLoader {
  ...
  parse(node, buffer) {
    ...
    const workerPath = `${Potree.scriptPath}/workers/GreyhoundBinaryDecoderWorker.js`;
    ...
    const nodeOffset = node.pcoGeometry.boundingBox.getCenter(new THREE.Vector3()).sub(node.boundingBox.min);
