...
BitReader = function (buf) {
  const buffer = new Uint32Array(buf);
  let bitOffset = 0;
  ...
  this.read = function (bits) {
    ...
    if ((bitOffset % 32) + bits <= 32) {
      const val = buffer[Math.floor(bitOffset / 32)];
      const leftGap = bitOffset % 32;
      const rightGap = 32 - (leftGap + bits);

      result = (val << leftGap) >>> (leftGap + rightGap);
    } else {
      let val = buffer[Math.floor(bitOffset / 32)];
      const leftGap = bitOffset % 32;
      const rightGap = (leftGap + bits) - 32;

      result = (val << leftGap) >>> (leftGap - rightGap);

      val = buffer[Math.floor(bitOffset / 32) + 1];
      result |= val >>> (32 - rightGap);
    }
